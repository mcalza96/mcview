...
def render(root: str, findings: dict) -> str:
    ...
    # ONE PATH SPACE, and it is the config's. `[project] root = "src"` means every path in the
    # file is relative to `src/`, so writing `src/app/page.tsx` names something that does not
    # exist and matches nothing. It was mixed: the test and auxiliary directories were already
    # computed against `src`, while the entry points, the door candidates and the `dirs`
    # fallback carried the repo-relative prefix. Two visible consequences, both found by an
    # agent meeting a project for the first time: `root = "src"` next to `dirs = ["src/"]`, a
    # root declaration matching zero files; and a `[surfaces]` block that crashed when
    # uncommented exactly as the skill says to.
    src_root = findings.get("src_root", ".")
    pref = "" if src_root == "." else src_root.rstrip("/") + "/"

    def in_config(rel: str) -> str:
        return rel[len(pref):] if pref and rel.startswith(pref) else rel

    def door_label(rel: str) -> str:
        """A name for a door, from its ROUTE and not from its file name.

        In a filesystem-routed framework every door is called `page.tsx`, so the basename gave
        thirty candidates all named `page` — duplicate keys, and a block that cannot be
        uncommented as it stands.
        """
        parts = [x for x in os.path.dirname(in_config(rel)).split("/")
                 if x and not (x.startswith("(") and x.endswith(")"))]
        parts = [x for x in parts if x not in ("app", "src", "pages")]
    ...
    dirs = [in_config(rel) for rel, _ in findings["entrypoints"][:10]]
    product = list(dirs)
    ...
    if findings["entrypoints"]:
        ...
        for rel, why in findings["entrypoints"][:10]:
            ...
            L.append(f'{svc} = "{in_config(rel)}"'.ljust(46) + f"# {why}")
    ...
    if findings.get("door_candidates"):
        ...
        for rel, n in findings["door_candidates"][:12]:
            ...
            linea = f'# "{door_label(rel)}" = ["{in_config(rel)}"]'
